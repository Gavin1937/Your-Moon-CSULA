    ...
    MoonDetection: async (fileObject, retType, callback) => {
        instance.ready.then(_ => {
            ...
            img.onload = function () {
                ...
                const p = instance._mrwasm_create_image_buffer(image.width, image.height);
                instance.HEAP8.set(image.data, p);
                
                // run moon_detection
                let detection_result_ptr = instance._mrwasm_detect_moon(p, image.width, image.height);
...
export default MoonRegistration;
